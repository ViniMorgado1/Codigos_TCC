...
# Funcao para tal = -0.5
cen5h1_50 <- function(n, rep=5000, B=1000){
  ...
  # Definindo os betas arbitrarios (muda de acordo com o cenario)
  betas <- matrix(data=c(-0.5,1.1,-0.5,
                         -0.5,1.1,-0.5,
                         4.6,-2.5,-0.5),
  ...
  resultados <- future_lapply(1:rep, function(i) {
    ...
    estatisticas_boot <- sapply(1:B, function(j) {
      ...
      # Ajustando modelos completo e reduzido
      mod_completo_boot <- DirichReg(y_boot ~ x12 + x13 | x12 + x13, data = dados_boot, model = 'alternative')
      mod_reduzido_boot <- DirichReg(y_boot ~ x12 | x12, data = dados_boot, model = 'alternative')
...
# Funcao para tal = -0.25
cen5h1_25 <- function(n, rep=5000, B=1000){
  # Definindo a semente principal
  set.seed(8261)
  ...
  # Definindo os betas arbitrarios (muda de acordo com o cenario)
  betas <- matrix(data=c(-0.5,1.1,-0.25,
                         -0.5,1.1,-0.25,
                         4.6,-2.5,-0.25),
                  nrow = 3,ncol = 3,byrow = TRUE)
  # Definindo a matriz X com as covariaveis
...
# Funcao para tal = 0.5
cen5h150 <- function(n, rep=5000, B=1000){
  # Definindo a semente principal
  set.seed(8261)
...
# Funcao para tal = 0.25
cen5h125 <- function(n, rep=5000, B=1000){
  # Definindo a semente principal
  set.seed(8261)
...
cen5h1_50(50,5000,1000)
cen5h1_50(100,5000,1000)

cen5h1_25(50,5000,1000)
cen5h1_25(100,5000,1000)

cen5h150(50,5000,1000)
cen5h150(100,5000,1000)

cen5h125(50,5000,1000)
cen5h125(100,5000,1000)